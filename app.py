...
with col1:
    time_input = st.text_input(
        "擊殺時間",
        placeholder="格式: 2025/08/11 16:30:45 或 08/11 16:30:45",
        help="支援多種格式：YYYY/MM/DD HH:MM:SS 或 MM/DD HH:MM:SS (可省略秒數)",
        key="time_input"
    )
...
if st.button("🎯 更新擊殺時間", use_container_width=True, type="secondary"):
    if not selected_boss:
        ...
        if parsed_time is None:
            st.error("⚠️ 時間格式不正確！請使用：YYYY/MM/DD HH:MM:SS 或 MM/DD HH:MM:SS (可省略秒數)")
        else:
            tracker.bosses[selected_boss]['last_killed'] = parsed_time.isoformat()
            if tracker.save_boss_data():
                st.success(f"✅ 已更新 {selected_boss} 的擊殺時間為 {parsed_time.strftime('%Y/%m/%d %H:%M:%S')}")
                # 清空輸入框
                st.session_state["time_input"] = ""
                st.rerun()
